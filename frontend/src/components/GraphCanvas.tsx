...
function activeNodeScore(node: NodeData, index: number, scoring: NodeScoringResult | null): number {
  return scoring?.scores[index] ?? node.risk_score ?? 0
}
...
function buildColors(
  ...
  for (let i = 0; i < nodes.length; i++) {
    const n = nodes[i]
    const score = activeNodeScore(n, i, scoring)
    const isHidden =
      hidden.has(n.entity_type) ||
      hiddenBehavioralRoles.has(n.behavioral_role) ||
      score < minRiskScore
...
function buildSizes(
  ...
  for (let i = 0; i < nodes.length; i++) {
    const n = nodes[i]
    const risk = activeNodeScore(n, i, scoring)
    if (
      hidden.has(n.entity_type) ||
      hiddenBehavioralRoles.has(n.behavioral_role) ||
      risk < minRiskScore
...
function buildLinkColors(
  ...
  for (let i = 0; i < edges.length; i++) {
    ...
    const isEdgeHidden =
      (srcNode !== null &&
        (hidden.has(srcNode.entity_type) ||
          hiddenRoles.has(srcNode.behavioral_role) ||
          activeNodeScore(srcNode, si ?? 0, scoring) < minRiskScore)) ||
      (tgtNode !== null &&
        (hidden.has(tgtNode.entity_type) ||
          hiddenRoles.has(tgtNode.behavioral_role) ||
          activeNodeScore(tgtNode, ti ?? 0, scoring) < minRiskScore))
...
export default function GraphCanvas({
  ...
  useEffect(() => {
    ...
    g.setPointSizes(
      buildSizes(nodes, hiddenEntityTypes, hiddenRoles, minRiskScore, sizeScale, scoring)
    )
    ...
    g.render(1)
    g.pause()
  }, [isReady, nodes, edges, rerunTrigger, minRiskScore]) // eslint-disable-line react-hooks/exhaustive-deps
