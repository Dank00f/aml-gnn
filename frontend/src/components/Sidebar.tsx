...
interface SidebarProps {
  ...
  minRiskScore: number
  onMinRiskScoreChange: (v: number) => void
  visibleRiskNodeCount: number
...
export default function Sidebar({
  ...
  function handleRiskPercentChange(value: string) {
    const parsed = Number(value)
    if (Number.isNaN(parsed)) return
    onMinRiskScoreChange(Math.max(0, Math.min(1, parsed / 100)))
  }
        ...
        <Flex direction="column" gap="1" p="3">
          ...
          <Flex direction="column" gap="1" px="1">
            ...
            <input
              ...
              step="0.05"
              value={minRiskScore}
              onChange={e => onMinRiskScoreChange(parseFloat(e.target.value))}
              aria-label="Минимальный risk score"
              style={{ width: '100%', accentColor: 'var(--red-9)' }}
            />
            <input
              type="number"
              min="0"
              max="100"
              step="5"
              value={Math.round(minRiskScore * 100)}
              onChange={e => handleRiskPercentChange(e.target.value)}
              aria-label="Минимальный risk score, проценты"
              style={{
                width: '100%',
                borderRadius: 'var(--radius-2)',
            ...
            />
            <Text size="1" color="gray">
              Показано {visibleRiskNodeCount} из {totalNodeCount}
            </Text>
          </Flex>
