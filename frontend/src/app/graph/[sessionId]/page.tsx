...
function GraphPageContent({ sessionId }: { sessionId: string }) {
  ...
  const [scoreMode, setScoreMode] = useState<ScoreMode>('rule_based')
  ...
  const ruleBasedScoring: NodeScoringResult | null = analysisResult?.node_scoring ?? null
  const gnnScoring: NodeScoringResult | null = analysisResult?.gnn_scoring ?? null
  const hasGnnScoring = gnnScoring !== null
  const scoringResult: NodeScoringResult | null =
    scoreMode === 'gnn' ? gnnScoring : ruleBasedScoring
  const effectiveScoreMode: ScoreMode = hasGnnScoring ? scoreMode : 'rule_based'
  const visibleRiskNodeCount = useMemo(
    () =>
      nodes.filter((node, index) => {
        const score = scoringResult?.scores[index] ?? node.risk_score ?? 0
        return score >= minRiskScore
      }).length,
    [nodes, minRiskScore, scoringResult]
  )
  ...
  const selectedNodeActiveScore =
    selectedNodeIndex >= 0
      ? (scoringResult?.scores[selectedNodeIndex] ?? selectedNode?.risk_score ?? 0)
      : null
    ...
    <Flex direction="column" style={{ height: '100vh', background: '#0d1117', overflow: 'hidden' }}>
      ...
      >
        ...
        {hasGnnScoring && (
          <SegmentedControl.Root
            size="1"
            value={effectiveScoreMode}
            onValueChange={value => setScoreMode(value as ScoreMode)}
          >
            <SegmentedControl.Item value="rule_based">Rule</SegmentedControl.Item>
            <SegmentedControl.Item value="gnn">GNN</SegmentedControl.Item>
          </SegmentedControl.Root>
        )}
      ...
      <Flex style={{ flex: 1, overflow: 'hidden' }}>
        <Sidebar
          ...
          visibleRiskNodeCount={visibleRiskNodeCount}
        ...
        {selectedNode && (
          <DetailPanel
            ...
            detectorResults={detectorResults}
            scoring={scoringResult}
            activeScoreMode={effectiveScoreMode}
            activeNodeScore={selectedNodeActiveScore}
